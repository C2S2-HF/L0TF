...
### This file generates Figures B.16, B.18 and B.20 in Appendix B.2.                           ###
...
nn=seq(32,256,32)
mode = "pre"
load(paste0("plot_results/",mode,"_tBlocks.RData"))
png(paste0("plots/",mode,"_TBlocks.png"),   pointsize = 25, width=900, height=1100)
...
## Figure B.18---------------------------
load(paste0("plot_results/",mode,"_tWave.RData"))
rm(ResultCp)
png(paste0("plots/",mode,"_TWave.png"), pointsize = 25, width=900, height=1100)
op <- par(mfrow=c(1,1), mar=c(4,2,3,2))
m <- matrix(c(1:6,7,7),nrow = 4, ncol = 2, byrow = TRUE)
layout(mat = m, heights = c(0.3, 0.3, 0.3, 0.2))
tmp=c("MSE", "nknot", "dP", "dS", "dH", "time")
...
## Figure B.20---------------------------
load(paste0("plot_results/",mode,"_tDoppler.RData"))
png(paste0("plots/",mode,"_TDoppler.png"),  pointsize = 22, width=1000, height=600)
op <- par(mfrow=c(1,1), mar=c(4,2,3,2))
